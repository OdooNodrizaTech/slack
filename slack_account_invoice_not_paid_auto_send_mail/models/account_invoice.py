...
class AccountInvoice(models.Model):
    ...
    def account_invoice_not_paid_auto_send_mail_item_real(self, mail_template_id, author_id):
        ...
        #action_custom_not_paid_send_mail_slack
        self.action_custom_not_paid_send_mail_slack()        
        #return
        return return_item
    
    @api.one    
    def action_custom_not_paid_send_mail_slack(self):
        web_base_url = self.env['ir.config_parameter'].sudo().get_param('web.base.url')
        
        attachments = [
            {                    
                "title": 'Se ha enviado por email el pago retrasado de la factura',
                "text": self.number,                        
                "color": "#36a64f",                                             
